...
> **This step is mandatory, not a convenience wrapper.** BlackBar has no self-service registration — every account is created by an admin, and the first admin only exists once `setup.sh` creates it. Starting the stack with `docker compose up -d` alone yields a running app that nobody can log in to (the backend logs a warning and `GET http://localhost:8000/health` reports `"setup_required": true`). If you already started the containers, just run `bash setup.sh` now — it is idempotent against a running stack.
...
All configuration is managed through a `.env` file in the project root. `setup.sh` writes most variables for you, but you can pre-populate or edit `.env` yourself. A reference `.env.example` at the repo root documents every supported variable — including optional ones (SendGrid, rate limits, upload limits) that `setup.sh` never writes. Don't hand-copy it to `.env` as your setup path: let `setup.sh` generate the secrets, then layer optional variables on top.
...
```bash
# Base directory for production data volumes (mongodb_data)
# Defaults to ./data if unset.
BLACKBAR_DATA_DIR=/var/lib/blackbar
```

The compose file resolves `${BLACKBAR_DATA_DIR:-./data}/mongodb` as the bind-mount source. Make sure the directory exists and is writable by the container's UID before bringing the production stack up. (The optional Grafana/Prometheus observability stack lives in `observability/` with its own compose file and volumes.)
...
If you have set `BLACKBAR_DATA_DIR` to a host directory that the container UID cannot write to, MongoDB will fail to start. Ensure the directory is owned by an appropriate user (`chown -R 999:999 $BLACKBAR_DATA_DIR/mongodb` for the official `mongo:5` image) or set permissive modes.

---

## Production Deployment

**First, run `bash setup.sh` on the production host** — it is the only mechanism that creates the first admin account (see [Quick Start](#quick-start)). Skipping it leaves a running stack with no way to log in. Non-interactive form for provisioning scripts:

```bash
ADMIN_EMAIL=admin@example.org ADMIN_PASSWORD='<strong password>' SEED_DEMO=N bash setup.sh
```

Then complete the following checklist:

- [ ] Set `ENVIRONMENT=production` in `.env`. The backend reads `ENVIRONMENT` (not `NODE_ENV`): production mode disables the public `/docs`, `/redoc`, and `/openapi.json` endpoints and refuses to start without a persisted `JWT_SECRET`. `docker-compose.prod.yml` also sets it on the backend service.
